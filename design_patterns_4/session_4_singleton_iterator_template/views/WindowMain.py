...
from controllers.KnightControllerFactory import KnightControllerFactory
from controllers.WarriorControllerFactory import WarriorControllerFactory
from controllers.RiderControllerFactory import RiderControllerFactory
...
class WindowMain:
    ...
    def __init__(self):
        ...
        self.controller_factories_by_actors: Dict[EnumActor, IControllerActor] = {
            EnumActor.Knight: KnightControllerFactory(),
            EnumActor.Rider: RiderControllerFactory(),
            EnumActor.Warrior: WarriorControllerFactory()
        }
        ...
        self.controller_game = ControllerGame.instance()
    ...
    def new_game(self):
        self.game = self.controller_game.new_game()
    ...
    def on_click_create_actor(self, button: ComponentButton):
        factory = self.controller_factories_by_actors[button.linked_enum]
        free_tiles = self.controller_game.get_free_pos_near_target(self.selected_building.position.copy(), Vector2D(x=3, y=3))
        print(free_tiles)
        if free_tiles:
            print("hi")
            controller = factory.create_actor_controller(free_tiles, self.selected_building.tribe)            
            self.controllers_actors.append(controller)
            self.game.actors.append(controller.actor)
            self.controller_game.init_actor(controller.actor)
    ...
    def update(self, delta_sec):
        ...
        make_turn: bool = False

        keys_pressed = key.get_pressed()
        if False: #any(keys_pressed):
        ...
        """
        if make_turn:
            self.controller_game.make_a_move()
