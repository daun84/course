...
from utils.iterator.CollectionObjectSurfaces import CollectionObjectSurface
...
class WindowMain:
    def __init__(self):
        ...
        ui_button_actor.add_listener_click(self.controller_game.on_click_create_actor)
        ...
        ui_button_end_turn.add_listener_click(self.controller_game.on_click_end_turn)
    ...
    def on_click_new_game(self, button: EventComponentButton):
        random.seed(time.time())
        self.new_game()

    def on_click_building(self, button: EventComponentButton):
        if button.linked_enum == self.game.turn_tribe:
            self.controller_game.selected_building = button.linked_item
    ...
    def update(self, delta_sec):
        ...
        self.controller_game.update_actors(delta_sec)
    ...
    def draw(self):
        ...
        objects_collection = CollectionObjectSurface(self.game.actors, self.game.buildings)

        for item, item_surface in objects_collection:
        ...
        img_font = font.render(f'Turn: {self.game.turn}   Tribe: {str(self.game.turn_tribe)}', True, (255, 255, 255))
