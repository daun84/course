...
class ControllerMap:
    ...
    def instance():
        if ControllerMap.__instance is None:
            ControllerMap.__instance = ControllerMap()
        return ControllerMap.__instance

    def __init__(self):
        if self.__instance is not None:
            raise Exception("Only one instance of ControllerMap is allowed")
        self.__instance = self

    def generate_map(self, game: Game):
        is_success = False
        try:
            game.map_size.x = 50
            game.map_size.y = 50
    ...
    def get_all_map_tiles_by_type(self, game: Game, map_tile_type: EnumMapTile):
        map_tiles = []
        try:
            for j in range(game.map_size.y):
                for i in range(game.map_size.x):
    ...
    def generate_initial_buildings(self, game: Game):
        is_success = False
        try:
            ground_map_tiles = self.get_all_map_tiles_by_type(game, EnumMapTile.Ground)
